...
class HandValueation {
  static CheckForPair(p: Player, cards: Card[]) {
    ...
    if (
      p.cards[0].value == p.cards[1].value ||
      cards_vl.includes(p.cards[0].value) ||
      cards_vl.includes(p.cards[1].value)
    ) {
...
function AddCard(card: Card, type: number) {
  ...
  if (card.value == 11) {
    ...
    img.src =
      "../assets/cards/" + card.value.toString() + "-" + card.suit + ".png";
  }
  if (type == 0) {
